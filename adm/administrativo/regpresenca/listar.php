...
<body>
	<div id="corpo">    	
        ...
        <div id="listar-coteudo">
    		<form id="register-form" name="register-form" method="post" action="confirmar.php"  >
			<button>confirma</button>
             <?php
				...
				echo '<thead>';
				echo '<tr><th>Marcar RA para <br>confirmar presença</th><th>NOME</th></tr>';
				echo '</thead>';
				echo '<tbody>';
				foreach($bd->ouvintesPagos($_POST['palestra']) as $row){
					echo "<tr><td><input type='checkbox' name='ra[]' value='" . $row['ra'] . "'><strong> " . $row['ra'] . "</strong></td>";
					echo "<td>".$row['nome'].'</td></tr>';
				}
				
				echo '</tbody>';
				echo '</table>';
				echo "<input type='hidden' name='palestra' value='" . $_POST['palestra'] . "'>";
			?>				
